...
for run_no,error_indexes in enumerate(training_error_indeces):
    ...
    result_arr.append({'Prompting Strategy':'Deterministic (1)','Run':run_no,'random seed':'NIL','Avg. BERT score F1':round(avg_f1,2),'Avg. BERT score Precision':round(avg_precision,2),'Avg. BERT score Recall':round(avg_recall,2)})
    with open(os.path.join('./prompting_results_test_set_1.csv'), mode='w',newline='',encoding='utf-8') as csvfile:
																# create csv writer
